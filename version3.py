...
class a1:
    ...
    def tcheker(me):
        ...
        fire=dac.find_one({"user.district":me["auth"]["district"],"auth.token":me["auth"]["token"]},
                          {"_id":1,"user.email":1,
    ...
    def qup(me,fire):    #Question database update
        ...
        dab=db["feed"]
        dac=dab[fire["auth"]["state"]]

        fire=dac.find_one({"q.district":me["q"]["district"],"q.election circles":me["auth"]["election circles"],"_id":ObjectId(me["q"]["hash"])},
                          {"_id":0,"auth.users":1})
        ########################################################################################
...
class maker():
    def questionmaker(me,fire):
                ...
                assert type(me["meta"]["tags"])==list and len(me["meta"]["tags"])<=4 
                for j in me["meta"]["tags"]:
                    assert len(j)<=10
                me1=nd()
                ...
                me2["q"]["district"]=fire["user"]["district"]
    ...
    def solutionmaker(me,fire):
            assert type(me["s"]["solution"])==str and len(me["s"]["solution"])<=3500
            assert type(me(["q"]["hash"]))==str
            dab=db["solutions"]
            dac=dab[fire["user"]["state"]]
